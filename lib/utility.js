...
function http2Send (req, res, body, nstatus = 200, headers = null) {
	...
	each(Object.keys(res._headers).filter(i => invalid(i) === false), i => {
		output[i] = res._headers[i];
	});

	if (headers !== null) {
		each(Object.keys(headers).filter(i => invalid(i) === false), i => {
			output[i] = headers[i];
...
function last (req, res, reject, err) {
	...
	if (err === void 0) {
		reject(new Error(req.allow.includes("GET") ? 405 : 404));
...
function ms (arg = 0, divisor = 1e6) {
	return `${arg / divisor} ms`;
}
...
function reduce (uri, map, arg) {
	if (map !== void 0) {
		each(Array.from(map.keys()).filter(route => {
			...
			if (route.includes(":") && route.includes("(") === false && arg.params === false) {
				arg.params = true;
				now = true;

				each(route.replace(regex.slashEnd, "").split("/"), (i, idx) => {
					if (i.includes(":")) {
