...
function stream (req, res, file = {etag: "", path: "", stats: {mtime: new Date(), size: 0}}) {
	res.header("content-length", file.stats.size);
	res.header("content-type", mime(file.path));
	res.header("last-modified", file.stats.mtime.toUTCString());

	if ((req.headers["if-none-match"] === file.etag) || (req.headers["if-none-match"] === void 0 && Date.parse(req.headers["if-modified-since"]) >= file.stats.mtime)) { // eslint-disable-line no-extra-parens
