...
const path = require("path"),
	...
	{all, clone, delimiter, last, ms, next, params, parse, partial, pipeable, reduce, stream, writeHead} = require(path.join(__dirname, "utility.js"));
...
class Woodland extends EventEmitter {
	constructor ({autoindex = false, cacheSize = 1e3, cacheTTL = 3e5, charset = "utf-8", defaultHeaders = {}, digit = 3, etags = true, indexes = ["index.htm", "index.html"], logging = {}, origins = ["*"], time = false, seed = 42} = {}) {
		...
		this.logging = {
			...
			format: logging.format || "%v %l %u %t \"%r\" %>s %b \"%{Referer}i\" \"%{User-agent}i\"",
	...
	clf (req, res) {
		const date = new Date(),
			pad = (arg = 0) => String(arg).padStart(2, "0"),
			time = (arg = 0) => {
				const neg = arg < 0;
		...
		return this.logging.format.replace("%v", req.headers.host)
			...
			.replace("%t", `[${date.getDate()}/${months[date.getMonth()]}/${date.getFullYear()}:${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())} ${time(date.getTimezoneOffset())}]`)
