...
class Woodland extends Base {
	...
	decorate (rq, rs) {
		...
		const parsed = parse(rq),
			remotes = "x-forwarded-for" in rq.headers ? rq.headers["x-forwarded-for"].split(/\s*,\s*/g) : [rq.connection.remoteAddress];
		let req, res;

		const attrs = {
				parsed: parsed,
				allow: this.allows(parsed.pathname),
				body: "",
				cors: rq.headers.origin !== void 0 && (this.origins.includes(all) || this.origins.includes(rq.headers.origin)),
				host: parsed.hostname,
				ip: remotes[remotes.length - 1],
				params: {}
			},
			resAttrs = {
				error: (status = 500, body, headers = {"cache-control": "no-cache"}) => {
					res.statusCode = status;

					return this.onerror(req, res, body !== void 0 ? body instanceof Error ? body : new Error(body) : new Error(http.STATUS_CODES[status]), headers);
				},
				json: (arg, status = 200, headers = {"content-type": "application/json"}) => {
					return res.send(JSON.stringify(arg), status, headers);
				},
				locals: {},
				raw: rs,
				redirect: (uri, perm = true) => res.send("", perm ? 301 : 302, {"location": uri}),
				status: function (arg) {
					res.statusCode = arg;
		...
		if (this.http2) {
			const write = rs.write.bind(rs);

			resAttrs._headers = {};
			resAttrs.statusCode = 200;
			resAttrs.hasHeader = key => key in res._headers;
			resAttrs.getHeader = key => res.hasHeader(key) ? clone(res._headers[key]) : void 0;
			resAttrs.getHeaders = () => clone(res._headers);
			...
			resAttrs.removeHeader = key => {
				if (res.hasHeader(key)) {
					delete res._headers[key];
				}
			};

			resAttrs.setHeader = (key, value) => {
				res._headers[key] = value;
			};

			resAttrs.header = resAttrs.setHeader;

			resAttrs.send = async (body, status = 200, headers = {}) => {
				let error = false,
			...
			resAttrs.writeHead = (status = res.statusCode, headers) => {
				res.statusCode = status;
			...
			resAttrs.end = rs.end.bind(rs);
			resAttrs.header = rs.setHeader.bind(rs);
			resAttrs.send = async (body, status = 200, headers = {}) => {
				if (res.headersSent === false) {
					...
					if (pipeable(req.method, output)) {
						writeHead(res, status, headers);
						output.on("error", () => void 0).pipe(res.raw);
					} else {
		...
		req = new Proxy(rq, {
			has: function (obj, prop) {
				return prop in attrs || prop in obj;
			},
			get: function (obj, prop) {
				return prop in attrs ? attrs[prop] : obj[prop];
			},
		...
		res = new Proxy(rs, {
			has: function (obj, prop) {
				return prop in resAttrs || prop in obj;
			},
			get: function (obj, prop) {
				return prop in resAttrs ? resAttrs[prop] : obj[prop];
			},
			set: function (obj, prop, value) {
				if (prop in resAttrs) {
					resAttrs[prop] = value;
	...
	route (...args) {
		...
		const [req, res] = this.decorate(...this.http2 === false ? args : [http2Normalize({headers: args[1]}, args[0]), args[0]]);
		...
		return new Promise((resolve, reject) => {
			...
			if (req.cors === false && req.headers.origin !== void 0) {
				res.error(403);
		...
		}).catch(err => this.onerror(req, res, err)).then(() => this.onfinish(req, res));
	}
