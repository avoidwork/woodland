...
class Woodland extends EventEmitter {
	constructor ({cacheSize = 1e3, cacheTTL = 3e5, defaultHeaders = {}, origins = ["*"], indexes = ["index.htm", "index.html"], etags = true, seed = 42} = {}) {
		super();
	...
	"static" (req, res, arg = "", start = process.cwd(), indexes = this.indexes) {
		if (arg.length === 0 || path.resolve(arg).indexOf(start) !== 0) {
			res.error(404);
			...
			fs.stat(arg, (e, stats) => {
				if (e !== null) {
					...
					stream(req, res, {path: arg, stats: stats});
					...
					fs.readdir(arg, {encoding: "utf8", withFileTypes: true}, (e2, files) => {
						if (e2 !== null) {
							...
							if (result.length === 0) {
								...
								fs.stat(result, (e3, rstats) => {
									if (e3 !== null) {
										res.error(500, e3);
									} else {
										stream(req, res, {path: result, stats: rstats});
									}
								});
							}
						}
