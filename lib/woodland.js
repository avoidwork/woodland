...
class Woodland extends EventEmitter {
	...
	decorate (req, res) {
		...
		req.body = "";
		req.cors = req.headers.origin !== void 0 && req.headers.origin.replace(/^http(s)?:\/\//, "") !== req.headers.host && (this.origins.includes(all) || this.origins.includes(req.headers.origin));
	...
	route (...args) {
		...
		if (req.cors === false && "origin" in req.headers && this.origins.includes(req.headers.origin) === false) {
			res.error(403);
