...
const path = require("path"),
	...
	{clone, each, http2Normalize, http2Send, normalize, params, parse, pipe, last, reduce, schemeless, wrapped} = require(path.join(__dirname, "utility.js")),
	all = "all",
	delimiter = ":";
...
class Woodland extends Base {
	...
	decorate (req, res) {
		...
		if (this.http2) {
			...
			res.send = (body, status = 200, headers = {}) => {
				let output = this.onsend(req, res, body, status, headers);
				const chunk = pipe(req.method, output);

				if (chunk === false && req.headers.range !== void 0 && headers["content-range"] === void 0) {
