...
class Woodland {
	...
	list (host = "all", method = "get", type = "array") {
		...
		if (type === "array") {
			const start = Array.from(this.middleware.get(host).get("ALL").keys());

			if (method !== "" && method.toLowerCase() !== "get") {
				result = [...start, Array.from(this.middleware.get(host).get(method.toUpperCase()).keys())];
			} else {
				result = start;
			}
		} else if (type === "object") {
			...
			this.middleware.get(host).get("ALL").forEach((value, key) => {
				start[key] = value;
			});
			...
			if (method !== "" && method.toLowerCase() !== "get") {
				this.middleware.get(host).get(method.toUpperCase()).forEach((value, key) => {
					result[key] = value;
				});
			}
		}
