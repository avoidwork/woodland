...
class Woodland extends Base {
	constructor (defaultHeaders, cacheSize, http2, cacheTTL, dtrace) {
		...
		if (this.dtp !== null) {
			this.dtp.addProbe("allows", "char *", "char *", "bool");
			this.dtp.addProbe("decorate", "char *", "char *");
			this.dtp.addProbe("error", "char *", "char *", "int");
			this.dtp.addProbe("route", "char *", "char *", "char *");
			this.dtp.addProbe("routes", "char *", "char *", "bool", "char *");
		}
	}
	...
	allows (uri, override = false) {
		...
		if (this.dtrace) {
			timer.stop();
			this.dtp.fire("allows", () => [uri, ms(timer.diff()), override]);
		}
	...
	decorate (req, res) {
		...
		const parsed = parse(req),
			remotes = "x-forwarded-for" in req.headers ? req.headers["x-forwarded-for"].split(/\s*,\s*/g) : [req.connection.remoteAddress];
		...
		if (this.dtrace) {
			timer.stop();
			this.dtp.fire("decorate", () => [req.parsed.href, ms(timer.diff())]);
		}
	}
	...
	onerror (req, res, err) {
		...
		if (this.http2 === false) {
			...
			if (this.dtrace) {
				timer.stop();
				this.dtp.fire("error", () => [req.parsed.href, ms(timer.diff()), status]);
			}
	...
	routes (uri, method, override = false) {
		...
		if (this.dtrace) {
			timer.stop();
			this.dtp.fire("routes", () => [uri, method, override, ms(timer.diff())]);
		}
