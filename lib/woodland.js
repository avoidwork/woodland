...
class Woodland extends EventEmitter {
	...
	decorate (req, res) {
		...
		res.error = (status = 500, body) => {
			const ev = "error",
				err = body !== void 0 ? body instanceof Error ? body : new Error(body) : new Error(http.STATUS_CODES[status]);
	...
	"static" (req, res, arg = "", start = process.cwd(), indexes = this.indexes) {
		if (req.method !== "GET" && req.method !== "HEAD" && req.method !== "OPTIONS") {
			res.error(405);
		} else if (arg.length === 0 || path.resolve(arg).indexOf(start) !== 0) {
			res.error(404);
		} else if ("expect" in req.headers) {
			res.error(417);
		} else {
			fs.stat(arg, {bigint: false}, (e, stats) => {
				if (e !== null) {
					res.error(404);
				} else if (stats.isDirectory() === false) {
					stream(req, res, {charset: this.charset, etag: this.etag(req.method, stats.ino, stats.size, stats.mtimeMs), path: arg, stats: stats});
				} else if (req.parsed.pathname.endsWith("/") === false) {
					res.redirect(`${req.parsed.pathname}/${req.parsed.search}`);
				} else {
					fs.readdir(arg, {encoding: "utf8", withFileTypes: true}, (e2, files) => {
						if (e2 !== null) {
							res.error(500, e2);
						} else {
							let result = "";
							...
							for (const file of files) {
								if (indexes.includes(file.name)) {
									result = path.join(arg, file.name);
									break search;
								}
							}
