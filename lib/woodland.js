...
const path = require("path"),
	...
	{clone, each, http2Normalize, http2Send, normalize, params, parse, pipeable, last, reduce, schemeless, wrapped} = require(path.join(__dirname, "utility.js")),
	all = "all",
	delimiter = ":";
			...
			res.send = (body, status = 200, headers = {}) => {
				...
				if (pipeable(req.method, output) === false) {
					if (req.headers.range !== void 0) {
						...
						this.partial(req, res, buffered, status, headers);
				...
				this.writeHead(res, status, headers);
