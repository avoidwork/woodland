...
class Woodland {
	...
	decorate (req, res) {
		...
		if (this.http2) {
			...
			res.setHeader = (key, value) => {
				res.tempHeaders[key] = value;
			};

			res.send = (body, status = 200, headers = null) => {
				if (res.writable) {
					const output = {":status": status};

					each(Object.keys(res.tempHeaders), i => {
						output[i] = res.tempHeaders[i];
					});
	...
	params (...args) {
		if (this.http2 === false) {
			const [req, pos = []] = args,
				uri = req.parsed.path.replace(regex.slashEnd, "").replace(regex.slashEnd, "").split("/");
