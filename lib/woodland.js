...
class Woodland {
	constructor (defaultHeaders, cacheSize, seed, coerce, http2, cacheTTL) {
		...
		this.cache = lru(cacheSize, false, cacheTTL);
		this.coerce = coerce;
		this.defaultHeaders = Object.keys(defaultHeaders).map(key => [key.toLowerCase(), defaultHeaders[key]]);
		this.http2 = http2;
		this.hashes = new Map();
		this.permissions = lru(cacheSize, false, cacheTTL);
		this.middleware = new Map();
		this.seed = seed;
	}

	allowed (method, uri, override = false) {
		return this.routes(uri, method, override).middleware.filter(i => this.notBlacklisted(i)).length > 0;
	}
	...
	blacklist (fn) {
		this.blacklisted.add(this.hash(fn.toString()));

		return this;
	}

	notBlacklisted (fn) {
		return this.blacklisted.has(this.hash(fn.toString())) === false;
	}
	...
	next (req, res, err, middleware, reject, next) {
		...
		if (iter.done === false) {
			...
			if (iter.done === false) {
				...
				this.last(req, res, reject, err); // Unhandled error
			}
		} else {
			this.last(req, res, reject, err); // Called next() in the last middleware
		}
	}
	...
	normalize (arg) {
		...
		obj.__proto__ = http.IncomingMessage.prototype;
		obj.__proto__.constructor = http.IncomingMessage;
	...
	_route (...args) {
		...
		return new Promise((resolve, reject) => {
			...
			const next = err => process.nextTick(() => this.next(req, res, err, middleware, reject, next));
			...
			each([this.middleware.get(all.toUpperCase()), this.middleware.get(method)], map => {
				if (map !== void 0) {
					each(Array.from(map.keys()).filter(route => {
						...
						middleware = middleware.concat(Array.from(map.get(route)).map(i => this.hashes.get(i)));
					});
				}
			});
	...
	use (...args) {
		...
		mhash = this.hash(fn.toString());
		mmethod.get(rpath).add(mhash);

		if (this.hashes.has(mhash) === false) {
			this.hashes.set(mhash, fn);
		}
