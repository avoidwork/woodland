...
class Woodland {
	...
	decorate (req, res) {
		...
		// Setting headers
		array.each(this.defaultHeaders, header => res.header(header[0], header[1]));
	...
	host (arg) {
		...
		array.each(this.hosts, (reg, host) => {
			let output = true;
	...
	routes (uri, host, method, override = false) {
		...
		if (cached) {
			...
			array.each([allMap.get(allMethod), allMap.get(method), hostMap.get(allMethod), hostMap.get(method)], map => {
				if (map) {
					array.each(Array.from(map.keys()).filter(route => {
						let now = false,
							valid;
						...
						if (hasParam.test(route) && route.indexOf("(") === -1 && !params) {
							...
							array.each(route.replace(startSlash, "").replace(endSlash, "").split("/"), (i, idx) => {
								if (isParam.test(i)) {
