...
class Woodland {
	...
	decorate (req, res) {
		...
		// Setting headers
		each(this.defaultHeaders, header => res.header(header[0], header[1]));
	...
	routes (uri, host, method, override = false) {
		...
		if (cached) {
			...
			each([allMap.get(allMethod), allMap.get(method), hostMap.get(allMethod), hostMap.get(method)], map => {
				if (map) {
					each(Array.from(map.keys()).filter(route => {
						...
						if (hasParam.test(route) && route.indexOf("(") === -1 && !params) {
							...
							each(route.replace(startSlash, "").replace(endSlash, "").split("/"), (i, idx) => {
								if (isParam.test(i)) {
