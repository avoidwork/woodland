...
class Woodland extends EventEmitter {
	...
	decorate (req, res) {
		...
		const parsed = parse(req),
			remotes = "x-forwarded-for" in req.headers ? req.headers["x-forwarded-for"].split(/\s*,\s*/g) : [req.connection.remoteAddress.replace(/^\:\:ffff\:/, '')];

		req.parsed = parsed;
		req.allow = this.allows(parsed.pathname);
		req.body = "";
		req.corsHost = req.headers.origin !== void 0 && req.headers.origin.replace(/^http(s)?:\/\//, "") !== req.headers.host;
