...
export class Woodland extends EventEmitter {
  ...
  error(
    req: IncomingMessage,
    res: ServerResponse,
  ): (status: number, body?: any, headers?: Record<string, string>) => void;
  ...
  send(
    req: IncomingMessage,
    res: ServerResponse,
  ): (body?: any, status?: number, headers?: Record<string, any>) => void;
  set(
    res: ServerResponse,
  ): (arg?: Record<string, any> | Map<string, any> | Headers) => ServerResponse;
  ...
  use(
    rpath: string | MiddlewareFunction,
    ...fn: (MiddlewareFunction | ErrorMiddlewareFunction | string)[]
  ): this;
}
