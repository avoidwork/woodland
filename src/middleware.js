import {DELIMITER, FUNCTION, GET, HEAD, LEFT_PAREN, SLASH, STRING, WILDCARD} from "./constants.js";
...
export function createMiddlewareRegistry (middleware, ignored, methods, cache) {
	...
	function register (rpath, ...fn) {
		...
		const nodeMethods = ["CONNECT", "DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT", "TRACE"];
		...
		return {ignore: ignoreFn, allowed: allowedFn, routes: routesFn, register: registerFn, list: listFn};
	}
	registerFn = register;
