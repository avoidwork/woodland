...
export function createLogger (config = {}) {
	...
	function extractIP (req) {
		...
		return connection && connection.remoteAddress || socket && socket.remoteAddress || "127.0.0.1";
	}
	...
	logFn = function (msg, logLevel = DEBUG) {
		...
		return {log: logFn, clfm: clfmFn, extractIP, logRoute: logRouteFn, logMiddleware: logMiddlewareFn, logDecoration: logDecorationFn, logError: logErrorFn, logServe: logServeFn};
	};
	...
	clfmFn = function (req, res) {
		...
		logEntry = logEntry.replace("%v", host)
			.replace("%h", ip)
			.replace("%l", logname)
			.replace("%u", username)
			.replace("%t", dateStr)
	...
	logDecorationFn = function (uri, method, ip) {
		return logFn(`type=decorate, uri=${uri}, method=${method}, ip=${ip}, message="Decorated request from ${ip}"`);
	};

	logErrorFn = function (uri, method, ip) {
		return logFn(`type=error, uri=${uri}, method=${method}, ip=${ip}, message="Handled error response for ${ip}"`, ERROR);
	};

	logServeFn = function (req, message) {
		return logFn(`type=serve, uri=${req.parsed.pathname}, method=${req.method}, ip=${req.ip}, message="${message}"`, ERROR);
	};
